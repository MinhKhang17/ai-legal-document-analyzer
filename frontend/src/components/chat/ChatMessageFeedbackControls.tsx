...
export function ChatMessageFeedbackControls({ messageId }: { messageId: string; language: 'en' | 'vi' }) {
  const { t } = useI18n();
  ...
  if (submitted) return <p className="mt-sm text-xs font-semibold text-emerald-700 dark:text-emerald-300">{t('chat.feedback.thanks')}</p>;
  return <div className="mt-sm border-t border-legal-border pt-sm dark:border-slate-700">
    <div className="flex gap-xs"><Button size="sm" variant="ghost" aria-label={t('chat.feedback.helpful')} onClick={() => void submit('THUMBS_UP')} disabled={saving}><ThumbsUp className="h-4 w-4" /></Button><Button size="sm" variant="ghost" aria-label={t('chat.feedback.notHelpful')} onClick={() => void submit('THUMBS_DOWN')} disabled={saving}><ThumbsDown className="h-4 w-4" /></Button></div>
    {rating === 'THUMBS_DOWN' && <div className="mt-sm space-y-sm"><p className="text-xs font-semibold">{t('chat.feedback.reasonPrompt')}</p><div className="flex flex-wrap gap-xs">{reasons.map((reason) => <button type="button" key={reason} onClick={() => setSelectedReasons((current) => current.includes(reason) ? current.filter((item) => item !== reason) : [...current, reason])} className={`rounded-full border px-2 py-1 text-[11px] ${selectedReasons.includes(reason) ? 'border-primary bg-primary text-white' : 'border-legal-border'}`}>{t(`chat.feedback.reason.${reason}`)}</button>)}</div><textarea className="form-field min-h-20 text-xs" maxLength={2000} placeholder={t('chat.feedback.commentPlaceholder')} value={comment} onChange={(e) => setComment(e.target.value)} /><Button size="sm" onClick={() => void submit('THUMBS_DOWN')} disabled={saving}>{t('chat.feedback.submit')}</Button></div>}
  </div>;
}
