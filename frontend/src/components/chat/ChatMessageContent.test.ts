...
describe('normalizeChatMarkdown', () => {
  ...
  it('preserves repeated claim-level citations', () => {
    const normalized = normalizeChatMarkdown(
      'Rủi ro cao [USER-1], không nên ký [USER-2]. Tuân thủ pháp luật [KB-1], bảo đảm an toàn [KB-2].',
    );

    expect(normalized.match(/\[USER-\d+\]/g)).toHaveLength(2);
    expect(normalized.match(/\[KB-\d+\]/g)).toHaveLength(2);
  });
});
