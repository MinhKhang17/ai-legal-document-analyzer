import { ArrowRight, Building2, Check, Eye, EyeOff, Lock, Mail, UserRound } from 'lucide-react';
  ...
  return (
    <Card className="mx-auto w-full max-w-[380px] px-5 py-4">
      ...
      >
        ...
        <div className="space-y-1">
          <label className="label-uppercase text-[8px]" htmlFor="workspace">
            {t('auth.workspace')}
          </label>
          <div className="relative">
            <Building2 className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-outline" />
            <input
              id="workspace"
              className="form-field h-9 py-1.5 pl-8 text-[13px]"
              value={formData.workspaceName}
              onChange={(event) => updateFormField('workspaceName', event.target.value)}
              required
            />
          </div>
        </div>
