...
export function RefundHistoryPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { setRefunds((await getMySubscriptionRefunds()).data ?? []); } catch (cause) { setError(cause instanceof Error ? cause.message : t('refund.errors.loadHistory')); } finally { setLoading(false); } }, [t]);
