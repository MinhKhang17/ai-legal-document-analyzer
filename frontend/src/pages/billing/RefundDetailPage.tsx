...
export function RefundDetailPage() {
  ...
  return <div><PageHeader title={t('refund.detail.title')} subtitle={`#${id}`} actions={<Link to="/billing/refunds"><Button variant="secondary">{t('actions.back')}</Button></Link>} /><Card>{error ? <div role="alert" className="text-error">{error} <Button onClick={() => void load()}>{t('common.retry')}</Button></div> : loading ? <p>{t('common.loading')}</p> : refund ? <dl className="grid gap-md md:grid-cols-2"><div><dt className="label-uppercase">{t('table.status')}</dt><dd><RefundStatusBadge status={refund.status} /></dd></div><div><dt className="label-uppercase">{t('refund.amount')}</dt><dd>{formatVndCurrency(refund.amount)}</dd></div><div><dt className="label-uppercase">{t('refund.transaction')}</dt><dd>#{refund.paymentTransactionId}</dd></div><div><dt className="label-uppercase">{t('table.created')}</dt><dd>{formatDisplayDate(refund.createdAt, '-')}</dd></div><div><dt className="label-uppercase">Ngân hàng</dt><dd>{refund.bankName || '-'} · {refund.accountNumber || '-'}</dd></div><div><dt className="label-uppercase">Xác nhận email</dt><dd>{refund.emailConfirmed ? 'Đã xác nhận' : 'Đang chờ xác nhận'}</dd></div><div className="md:col-span-2"><dt className="label-uppercase">{t('refund.reason')}</dt><dd>{refund.reason}</dd></div>{refund.legalTicketId && <div className="md:col-span-2"><Link to={`/tickets/${refund.legalTicketId}`}><Button>Chat với admin về hoàn tiền</Button></Link></div>}{refund.adminNote && <div className="md:col-span-2"><dt className="label-uppercase">{t('refund.adminNote')}</dt><dd>{refund.adminNote}</dd></div>}</dl> : <p>{t('refund.detail.notFound')}</p>}</Card></div>;
}
