...
public class FinancialAuditService {
    ...
    @Transactional public void record(String action,User actor,String entityType,String entityId,String oldJson,String newJson,String reason,String requestId){ repository.save(FinancialAuditLog.builder().action(action).actor(actor).entityType(entityType).entityId(entityId).oldValuesJson(oldJson).newValuesJson(newJson).reason(reason).requestId(requestId).build()); }
