...
public interface ExpertRevenueStatementRepository extends JpaRepository<ExpertRevenueStatement,String>{
 ...
 @Lock(LockModeType.PESSIMISTIC_WRITE) @Query("select s from ExpertRevenueStatement s where s.id=:id") Optional<ExpertRevenueStatement> lockById(@Param("id") String id);
}
