...
import com.analyzer.api.entity.CommissionPolicy;
...
public interface CommissionPolicyRepository extends JpaRepository<CommissionPolicy, String> {
    @Query("select p from CommissionPolicy p where p.status in :statuses and p.effectiveFrom<=:date and (p.effectiveTo is null or p.effectiveTo>=:date) order by p.effectiveFrom desc")
    List<CommissionPolicy> applicable(@Param("date") LocalDate date,
            @Param("statuses") Collection<CommissionPolicyStatus> statuses);
    ...
    @Query("select p from CommissionPolicy p where p.id=:id")
    Optional<CommissionPolicy> lockById(@Param("id") String id);
}
