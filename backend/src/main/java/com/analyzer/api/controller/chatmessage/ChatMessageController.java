package com.analyzer.api.controller.chatmessage;
...
public class ChatMessageController {
    ...
    public ResponseEntity<ApiResponseDTO<SendMessageResponse>> sendFirstMessageInWorkspace(
        ...
        SendMessageResponse data = chatMessageService.sendMessageInWorkspace(currentUser.getId(), workspaceId, request);
    ...
    public ResponseEntity<ApiResponseDTO<SendMessageResponse>> sendMessageInChatSession(
            ...
            @Valid @RequestBody SendMessageRequest request) {
        SendMessageResponse data = chatMessageService.sendMessageInChatSession(currentUser.getId(), chatSessionId, request);
        return ResponseEntity.ok(ApiResponseDTO.success("Message processed successfully", data));
    }
    ...
    public ResponseEntity<ApiResponseDTO<PageResponse<ChatMessageResponse>>> getMessagesByChatSession(
        ...
        PageResponse<ChatMessageResponse> data = chatMessageService.getMessagesByChatSession(currentUser.getId(), chatSessionId, page, size);
    ...
    public ResponseEntity<ApiResponseDTO<ChatMessageResponse>> getMessageDetail(
        ...
        ChatMessageResponse data = chatMessageService.getMessageDetail(currentUser.getId(), messageId);
    ...
    public ResponseEntity<ApiResponseDTO<ChatMessageFeedbackResponse>> submitFeedback(
        ...
        ChatMessageFeedbackResponse data = chatMessageService.submitFeedback(currentUser.getId(), messageId, request);
    ...
    public ResponseEntity<ApiResponseDTO<Void>> removeFeedback(
            @AuthenticationPrincipal UserDetailsImpl currentUser, @PathVariable String messageId) {
        chatMessageService.removeFeedback(currentUser.getId(), messageId);
