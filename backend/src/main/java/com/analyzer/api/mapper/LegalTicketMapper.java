...
public interface LegalTicketMapper {
    ...
    @Mapping(target = "userDisplayName", expression = "java(ticket.getCreatedBy() != null ? ticket.getCreatedBy().getFirstName() + \" \" + ticket.getCreatedBy().getLastName() : null)")
    @Mapping(target = "userEmail", source = "createdBy.email")
    @Mapping(target = "userPhone", ignore = true)
    @Mapping(target = "selectedDocumentIds", ignore = true)
    @Mapping(target = "sharedProfileFields", ignore = true)
    @Mapping(target = "aiQuestion", source = "question")
    @Mapping(target = "aiAnswerSummary", source = "answer")
    @Mapping(target = "aiIntent", source = "legalDomain")
    @Mapping(target = "aiRiskLevel", source = "riskLevel")
    @Mapping(target = "aiConfidence", source = "confidenceScore")
    @Mapping(target = "classifiedById", source = "classifiedBy.id")
    @Mapping(target = "proposedExpertId", source = "proposedExpert.id")
    @Mapping(target = "proposedExpertName", expression = "java(ticket.getProposedExpert() != null ? ticket.getProposedExpert().getFirstName() + \" \" + ticket.getProposedExpert().getLastName() : null)")
    @Mapping(target = "paymentStatus", source = "customerPaymentStatus")
